...
class RequestLimit(object):
    def __init__(self):
        ...
        self.github_repo_path = "/Users/mopatel/Projects/ccgf-qastaging-hawk-config"
        self.common_json = json.load(open(f"{self.github_repo_path}/deployment/common.json"))
        ...
        self.container_service_mapping = json.load(open("/Users/mopatel/Projects/ccgf-scripts/platform-deployment-config/container-service-mapping.json"))
    ...
    def get_avg_cpu_of_service_per_namespace(self, namespace):
        ...
        for i in response["data"]["result"]:
            ...
            if container and container not in self.ignore_contianers:
                ...
                self.result[container]["request_count"]["cpu"] = float(f"{Decimal(f'{cpu:.2g}'):f}")
        ...
        for i in response["data"]["result"]:
            if not i.get("metric").get("container") in self.result:
                ...
                self.result[i.get("metric").get("container")]["request_count"][
                    "memory"] = f'{round(float(i.get("value")[1]) * 9.5367431640625e-7)}' + "Mib"
            else:
